...
def update_post_content(post_id, updated_content):
    ...
    if response.status_code == 200:
        ...
        print(f"Failed to update post content for post {post_id}: {response.status_code}, {response.text}")
...
def process_post_content(post_id):
    ...
    # Regex to find the references list under "References"
    pattern = r'(<p>References</p>\s*(?:<p>&nbsp;</p>\s*)?<ol class="wp-block-list">.*?</ol>)'
    ...
    if match:
        ...
        list_block_match = re.search(r'<ol class="wp-block-list">(.*?)</ol>', full_block, flags=re.DOTALL | re.IGNORECASE)
        if list_block_match:
